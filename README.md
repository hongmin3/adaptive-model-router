...
8. 같은 `group`에 속한 규칙은 **합산되지 않고 가장 강한 하나만 반영**됩니다. "자동화 기능을 구현해줘"는 구현·자동화·기능요청 세 규칙에 동시에 걸리지만 여전히 평범한 기능 하나이므로, 합산하면 어휘 규칙을 추가할 때마다 등급이 부풀려집니다. 범위(`multiple_files`, `project_wide_analysis`)와 위험(`high_impact_operation`)은 실제로 누적되므로 그룹 없이 합산합니다.

### 규칙 어휘는 실제 사용 기록에서 뽑습니다

규칙을 상상해서 쓰면 작성자가 이미 놓친 것과 같은 것을 놓칩니다. 이 프로젝트의 어휘는 실제 세션 기록에서 사용자가 실제로 친 프롬프트를 모아, 어느 규칙에도 안 걸린 것들의 단어 빈도를 세어 도출했습니다. 그 결과 미분류 비율이 **78% → 38%** 로 내려갔고, 옮겨간 대부분은 "근거를 몰라서 FAST/LOW"에서 "평범한 작업이라 BALANCED/MEDIUM"으로 이동한 것입니다.

가장 컸던 구멍은 **고장 신고**였습니다. 모든 진단 규칙이 `오류`/`에러`/`error`/`버그`/`실패` 라는 *단어*를 요구했는데, 사람이 실제로 쓰는 가장 흔한 버그 신고는 "이게 안 되는데" 입니다. `malfunction_report` 규칙이 부정 표현(`안 되는`, `~지 않는데`, `반응이 없`, `누락`, `비어있` 등)을 받습니다. 사동형(`안 되게 해줘`)은 기능 요청이므로 제외합니다.
...
### 화면별 동작 모드 — 터미널은 차단, 데스크톱은 안내

`~/.claude/settings.json`은 **사용자 단위**라 Hook을 한 번 등록하면 Claude Code의 모든 화면(터미널, 데스크톱 앱, VS Code 확장, SDK)에서 발동합니다. 하지만 "같은 Prompt를 다시 제출해 승인"이 자연스러운 건 터미널뿐입니다. 그래서 화면마다 **말하는 크기**를 다르게 합니다.
...
판정은 Claude Code가 하위 프로세스에 내려주는 `CLAUDE_CODE_ENTRYPOINT` 환경변수로 합니다(터미널은 미설정 → `cli`로 해석). `*`는 이름이 없는 화면의 기본값이며, 출하 기본은 `off`라서 Anthropic이 새 화면을 추가해도 이름을 적어 주기 전까지는 조용합니다. 오타 같은 잘못된 모드 값도 `off`로 떨어집니다 — 승인할 수 없는 화면에서 조용히 `block`이 되는 쪽이 훨씬 나쁘기 때문입니다.
...
#### 데스크톱 앱에서 보이는 모습 (`advise`)

Prompt는 그대로 실행되고, 아래 내용이 알림으로만 표시됩니다.
...
이 Prompt는 그대로 실행됩니다. 추천을 쓰려면 /model에서 위 모델과 Effort 값을 바꾸세요.
```

`advise` 모드는 승인 상태를 저장하지 않습니다. 막은 것이 없으니 승인할 것도 없습니다.

#### `block`이 실제로 막는 경우는 좁습니다

승인을 받으려면 **글자 하나 다르지 않은 같은 문장**을 다시 보내야 합니다. 그런데 예상 못 한 화면이 뜨면 사람은 질문을 고쳐 쓰고, 고쳐 쓴 문장은 또 "첫 제출"이라 다시 막힙니다. 실제로 이 상태에 빠진 터미널 세션이 관측됐습니다 — 턴이 하나도 성사되지 않은 채 서로 다른 프롬프트 3개가 연속으로 막혔고, 세 번 다 추천은 `haiku/LOW`, 이유는 `no matching routing evidence`였습니다.

그래서 `block`은 **말할 근거가 있고, 현재 설정을 알고, 그 둘이 다를 때만** 막습니다.

| 상황 | 동작 |
|---|---|
| 매칭된 규칙이 없음 (`no matching routing evidence`) | 안내만. 이유를 못 대는 확인 화면은 가장 나쁜 개입입니다 |
| 현재 모델을 모름 (세션 첫 프롬프트 등) | 안내만 |
| 이미 추천과 같은 설정 | 아무 말 없음 |
| 근거 있음 + 현재 설정 알려짐 + 다름 | **차단** |

"현재 모델을 모르면 안내만"은 특히 중요합니다. 막으면 턴이 성사되지 않고, 턴이 없으면 transcript에 모델이 기록되지 않고, 그러면 영원히 모델을 모릅니다 — **막는 상태가 스스로를 유지합니다.** 그 고리를 끊습니다.

#### 탈출구

프롬프트에 `라우터 꺼`, `router off`, `그냥 해`, `바로 해` 중 하나가 들어 있으면 Router는 그 프롬프트를 그대로 통과시킵니다. 이전 문장을 복원하지 않아도 빠져나올 수 있는 길이 필요하기 때문입니다.

#### 이미 최적이면 아무 말도 안 합니다

현재 모델과 Effort가 추천과 **둘 다** 같으면 어떤 모드든 조용히 통과합니다. 이미 맞는 설정을 확인받자고 막으면, 그 화면은 곧 아무 정보도 없는 것으로 학습되기 때문입니다. 단, 모델이나 Effort 중 하나라도 알 수 없으면(세션 첫 Prompt 등) "일치"로 보지 않고 정상 동작합니다 — 없는 것은 같은 것이 아닙니다.
...
| Router 건너뛰기 | `라우터 꺼`, `라우터 끄고`, `router off`, `그냥 해`, `바로 해` |
...
| 데스크톱 앱/VS Code에서 차단 화면이 뜬다 | `hook.claude_modes`에서 그 화면이 `block`으로 되어 있는지 확인. 기본값은 `advise`입니다 |
| 터미널인데 Router가 안 뜬다 | `echo $env:CLAUDE_CODE_ENTRYPOINT`로 값을 확인하고 `hook.claude_modes`에 `"<값>": "block"` 추가 |
| 추천이 아예 안 뜬다 | 현재 설정이 이미 추천과 같으면 조용히 통과합니다. `--no-exec --debug`로 추천값을 직접 확인하세요 |
| 계속 막혀서 빠져나올 수 없다 | 프롬프트에 `라우터 꺼`를 넣으면 즉시 통과합니다. 승인은 **완전히 같은 문장**을 다시 보내야 성립하므로, 문장을 고쳐 쓰면 다시 막힙니다 |
...
현재 회귀 묶음은 한국어·영어와 단순 편집, 구현, 진단, 광범위 분석, 다중 파일 작업, 위험 작업, 문자열 인용, 명시적 effort 지시를 포함한 **100개 프롬프트**를 검사합니다. 그중 29개는 프로필·reasoning에서 멈추지 않고 실제 Claude alias와 effort까지 확인하며, 프로필 4종 × reasoning 6단계를 모두 덮는지도 테스트가 스스로 검사합니다. 전체 테스트는 **156개**이고, 실행 파일 tier 추출은 실제 222MB 바이너리 대신 합성 fixture로 검사하므로 결과가 이 PC의 설치 상태에 좌우되지 않습니다.
...
- `hook.claude_modes`에서 Claude Code 화면별 동작 모드 지정 (`block`/`advise`/`off`, `*`는 기본값)
